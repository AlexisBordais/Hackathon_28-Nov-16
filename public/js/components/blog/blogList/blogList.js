...
((app) => {
    ...
    app.component('blogList', {
        ...
        controller: ['PostsService', '$stateParams', '$state', function(PostsService, $stateParams, $state) {
            // Récupérer dans le stateParams le lieu tester si lieu existe donc faire le filtre sinon pas de filtre
            // faire la requet la normale (tous les projects)
            if ($stateParams.lieu && $stateParams.langage) {
                PostsService.filter($stateParams).then((res) => {
                    this.posts = res.data
                })

            } else if ($stateParams.lieu || $stateParams.langage) {
                PostsService.filter($stateParams).then((res) => {
                    this.posts = res.data
                })
            } else {
